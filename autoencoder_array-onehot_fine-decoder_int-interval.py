...
if pretraining:
  ...
  ### Fit the model
  history = meta_model.fit(U_k, [In,In,In,In,In,In], epochs=epoch_pretrain, verbose=verbose, shuffle=False, batch_size=batch_size)
...
if len(sys.argv) > 5:
  if sys.argv[5] == 'BER':
    C = np.round(model_encoder.predict(u_k)).astype('int')
    print('codebook C is Linear? ', utils.isLinear(C))
    BER[f"auto-array-one-dec"], BLER[f"auto-array-one_dec"] = utils_ML.bit_error_rate_NN_decoder(N, k, C, nb_pkts, e0, e1,model_decoder, 'one', train_epsilon)

print(BER[f"auto-array-one-dec"])
...
# \Python3\python.exe autoencoder_array-onehot_fine-decoder_int-interval.py BAC 4 2 2 BER 1000 medium