...
def meta_model_generator(k,channel,model_enc,model_dec,round,epsilon_t):
  ...
  if round:
    # x = Lambda(utils_ML.round_sigmoid, name='rounding_layer', arguments={'a': 30})(encoded_bits)
    x = Lambda(utils_ML.gradient_stopper, name='rounding_layer')(encoded_bits)
  else:
    # x = Lambda(utils_ML.round_sigmoid, name='rounding_layer', arguments={'a': 10})(encoded_bits)
    x = encoded_bits
  if channel == 'BSC':
  ...
  # decoded_bits = Lambda(utils_ML.gradient_stopper, name='decoder_model')(model_dec(inputs=noisy_bits))
  # decoded_bits_1 = Lambda(utils_ML.gradient_stopper, name='decoder_model_1')(model_dec(inputs=noisy_bits_1))
  # decoded_bits_2 = Lambda(utils_ML.gradient_stopper, name='decoder_model_2')(model_dec(inputs=noisy_bits_2))
  # decoded_bits_3 = Lambda(utils_ML.gradient_stopper, name='decoder_model_3')(model_dec(inputs=noisy_bits_3))
  # decoded_bits_4 = Lambda(utils_ML.gradient_stopper, name='decoder_model_4')(model_dec(inputs=noisy_bits_4))
  # output_bits = Lambda(utils_ML.gradient_stopper, name='output_rounding_layer')(decoded_bits)
...
for a in range(iterations+1):
  ...
  ### Fit the model
  history = meta_model.fit(U_k, [U_k, U_k, U_k, U_k, U_k, U_k], epochs=epoch_int, verbose=verbose, shuffle=False, batch_size=batch_size, callbacks=cbks)
  ...
  if len(sys.argv) > 5 and a%2==1 or a==iterations:
    if sys.argv[5] == 'BER':
      C = np.round(model_encoder.predict(u_k)).astype('int')
      print('codebook C is Linear? ', utils.isLinear(C))
...
plt.title(f'array-array - Training results vs No. epoch - {nb_pkts} pkts')
...
## TEST
if len(sys.argv) > 5:
  utils.plot_BAC(f'BER N={N} k={k} - ARRAY-ARRAY - {nb_pkts} pkts', BER, k / N)
...
# \Python3\python.exe autoencoder_array-array_fine-decoder.py BAC 8 4 0 BER 1000 medium