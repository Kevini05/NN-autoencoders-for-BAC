...
if channel == 'BSC':
  ...
  inputs_decoder = keras.Input(shape = N)
x = Dense(units=S*2**(k), activation='Mish')(inputs_decoder)
...
if channel == 'BSC':
  ...
  noisy_bits = Lambda(BAC_noise, arguments={'epsilon0':train_epsilon})(inputs_meta)
...
def BER_NN(codebook,nb_pkts=100):
  ...
  BER['dec'],a = utils_ML.bit_error_rate_NN(N, k, codebook, nb_pkts, e0, e1,model_decoder, 'one')
...
# \Python3\python.exe decoder.py BAC 8 4 200 BER 1000